...
export const PlacemarkSpecPlus = Joi.object()
  .keys({
    ...
    name: Joi.string().required().example("Black Forest"),
    categoryid: IdSpec,
    description: Joi.string().allow(null).empty("").example("nice forest"),
    location: Joi.string().allow(null).empty("").example("america"),
    locLat: Joi.number().allow(null).example(49.0134),
    locLng: Joi.number().allow(null).example(12.1016),
    weather: Joi.string().allow(null).empty("").example("hot"),
    image: Joi.string().allow(null).empty("").example("cloudinary"),
    _id: IdSpec,
    __v: Joi.number(),
}).label("PlacemarkPlus");
